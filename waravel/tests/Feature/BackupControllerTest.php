...
class BackupControllerTest extends TestCase
{
    ...
    public function test_restore()
    {
        ...
        $this->app->bind('exec', function ($command, &$output, &$resultCode) {
            if (strpos($command, 'DROP SCHEMA') !== false) {
                $resultCode = 0;
            } elseif (strpos($command, 'psql') !== false) {
                $resultCode = 0;
                $output = [
                    '?column?',
                    '----------',
        ...
        $mockZip->shouldReceive('open')
            ->andReturn(true);
        $mockZip->shouldReceive('extractTo')
            ->andReturn(true);
        $mockZip->shouldReceive('close');

        $this->app->instance(\ZipArchive::class, $mockZip);

        $response = $this->postJson("/api/backups/restore/{$zipFile}");
        ...
        $responseOutput = $response->json('output');
        $this->assertStringContainsString('1', implode(' ', $responseOutput));
        ...
        $this->assertDirectoryDoesNotExist($storageBackupPath);
    }
